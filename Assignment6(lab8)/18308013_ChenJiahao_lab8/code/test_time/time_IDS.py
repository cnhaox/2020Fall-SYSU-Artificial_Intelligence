    ...
    for i in range(len(FileText)):
        for j in range(len(FileText[i])):
            if FileText[i][j]=='S':
                ...
                EndNode=node(i,j,0)
    timer1=Timer("Ierative_Deepening_Search(FileText, StartNode, EndNode)","from __main__ import Ierative_Deepening_Search, FileText, StartNode, EndNode")
    total_time=timer1.timeit(500)
    print("IDS after 500 times: "+str(total_time)+"secs")
    #main()