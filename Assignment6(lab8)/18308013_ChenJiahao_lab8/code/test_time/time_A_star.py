    ...
    for i in range(len(FileText)):
        for j in range(len(FileText[i])):
            if FileText[i][j]=='S':
                ...
                EndNode=node(i,j,0)
    #main()
    timer1=Timer("A_star_search(FileText,StartNode,EndNode)","from __main__ import A_star_search, FileText, StartNode, EndNode")
    total_time=timer1.timeit(500)
    print("A* after 500 times: "+str(total_time)+"secs")
    #main()