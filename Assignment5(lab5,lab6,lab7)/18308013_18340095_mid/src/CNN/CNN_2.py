...
def main():
    ...
    model = build_model()
    train_datagen = ImageDataGenerator(width_shift_range=0.2,  # 水平方向上平移的范围，相对于总宽度的比例
                                       height_shift_range=0.2,  # 垂直方向上平移的范围，相对于总高度的比例
                                       horizontal_flip=True,  # 随机将一半图像水平翻转
                                       )
    train_gen = train_datagen.flow(train_images, train_labels, batch_size=64)
    history = model.fit_generator(generator=train_gen, epochs=50, validation_data=(val_images, val_labels))

    model.save('cifar10_2.h5')  # _2为最原始的CNN使用数据增强
