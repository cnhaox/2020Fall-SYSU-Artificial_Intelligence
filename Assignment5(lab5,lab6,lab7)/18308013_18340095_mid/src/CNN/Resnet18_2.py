...
def main():
    ...
    opt = optimizers.SGD(lr=1e-1, momentum=0.9, neterov=False)
    model.compile(optimizer=opt, loss='categorical_crossentropy', metrics=['accuracy'])

    # 数据增强
    train_datagen = ImageDataGenerator(rotation_range=40,       # 0~180，图像随机旋转的角度范围
    ...
    history = model.fit_generator(generator=train_gen, epochs=200, validation_data=(val_images, val_labels))

    model.save('cifar10_resnet2.h5')
